...
def uniq_values_in_feature(data, feature):
    '''Checks each column in dataframe. Then prints out
...
def missing_vals(data, percent = True):
    ...
    if percent:
        ...
        for i in features:
            print(i, '  ----> ', (data[i].isnull().sum()/len(data)*100).round(1),'%')
    else:
        print('Proportion missing of total')
        print(data.isnull().sum()/len(data))

def missing_count(data):
    ''' Prints out missing value in each column as count and a percentage of the number of rows in input dataframe
    ...
    print('Missing Data')
    for i in features:
        print(i, '  ----> ', (data[i].isnull().sum()), '(', (data[i].isnull().sum()/len(data)*100).round(1),'%', ')')
...
def case_selection(data, case = 'lower'):
    '''Makes strings upper or lowercase

       Parameters:
       data: pandas dataframe object
       case: 'upper' or 'lower'. The default is lower case'''
...
def drop_missing (data, target):
    ''' drop missing value, empty strings and zero values in the target column of the dataframe input 
...
def drop_zero (data, target):
    '''  zero values in the target column of the dataframe input 
    and returns the result
    ...
    int_rows = data.shape[0]
    if target in data.columns:
        print ('\n', 'Dropped %d rows with zero %s values' %(data[data[target] == 0].shape[0], target))
        return data[data[target] > 0]
    else:
        print(target, ' is not a feature')
        return data
...
#### NLP spec from here some above
####
def check_non_ascii(data):

    '''code is to check which and explore which characters that are still not ascii
        ...
        if data[i].dtype == object: 
            ...
            for row in data[i]:   
                try:
                    ...
                    print('Error found in row %d in %s' %(row_cnt, i))
                finally:
                    row_cnt +=1
...
def conv_to_ascii(data):
    '''converts unicode to ascii
        ...
        Parameters:
            data: Pandas dataframe
    '''
    data_x = copy.copy(data)
    for i in data_x.columns:
        if data_x[i].dtype == object:
            data_x[i]= data_x[i].apply(lambda x: unidecode (x)) #using unidecode module
    return data_x         

def cat_chart(data, feature ):
    ''' Draws a horizontal bar chart 
        Parameters:
        data: Pandas dataframe
        feature: input string
    '''
    stor= dict(data[feature].value_counts())
    
    if len(stor) > 11:
        stor1 = {}
        cnt = 0
        for ke, val, in stor.items():
            if cnt <11:
                stor1[ke] = val
                cnt += 1  
    else:
    ...
    pos = np.arange(len(key)) + 0.5
    ax.barh(pos, valu, align = 'center', linewidth=1, edgecolor="black", facecolor = 'pink')
...
def subcat_chart(data, feature ):
    ''' Draws a horizontal bar chart using Seaborn
        Parameters:
        data: Pandas dataframe
        feature: input string
    '''
    stor= data[feature].value_counts().reset_index()
    stor['index']= stor['index'].astype(str)

    stor = stor[:10]
                     
    fig, ax = plt.subplots(figsize = (5,5))
    ax = sns.barplot(y ='index', x =feature, data = stor, linewidth=1, edgecolor="black")
...
def remove_punc2(data):
    '''Removes punctuation and symbols ('!"#$%&\'()*+,-./:;<=>?@[\\]^_`{|}~') from a string and replace it with a space
    Parameters: 
        data: Pandas dataframe object
    '''
    data_x = copy.copy(data)
    for i in data_x.columns:
        if data_x[i].dtype == object:
            for x in punctuation:
                data_x[i] = data_x[i].apply(lambda strg: strg.replace(x, " "))
            
    return data_x

def remove_punc(data):
    '''Removes punctuation and symbols ('!"#$%&\'()*+,-./:;<=>?@[\\]^_`{|}~') from a string
        Parameters: 
            data: Pandas dataframe object
    '''
    data_x = copy.copy(data)
    for i in data_x.columns:
        if data_x[i].dtype == object:
            data_x[i] = data_x[i].apply(lambda x: ''.join([char if char not in punctuation else " " for char in x]))
...
def remove_stopwords(data):
    ...
    for i in data_x.columns:
        if data_x[i].dtype == object:
            data_x[i] = data_x[i].apply(lambda x: ' '.join([wrd for wrd in x.lower().split(' ') if wrd not in stop_words]))
